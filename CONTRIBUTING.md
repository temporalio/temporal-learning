...
The Temporal learn site uses [Docusaurus 2](https://docusaurus.io/) version 2, which is a static website generator.
...
In the Markdown files, [Snipsync](https://github.com/temporalio/snipsync) snippets appear like this:
...
To preview snipsync snippets, run `yarn getsnips`, which inserts the snippet contents (in this case, from [`samples-typescript`](https://github.com/temporalio/samples-typescript/blob/75bdcd613bd24f8f357cb96d1b83051353c5685a/hello-world/src/client.ts#L1)) between the `SNIPSTART` and `SNIPEND` tags.

Before committing, run `yarn clearsnips`to remove the snippets.

## Pull requests

You can preview the changes made by your PR by clicking "Details" next to the Vercel deploy-preview check.

When we merge your PR, a new build automatically occurs and your changes publish to [https://learn.temporal.io](https://docs.temporal.io).
