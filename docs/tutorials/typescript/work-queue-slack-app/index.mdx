...
### Test the Workflow with Jest framework
...
[docs/tutorials/typescript/work-queue-slack-app/code/temporal-application/src/__tests__/workqueue.test.ts](https://github.com/temporalio/temporal-learning/blob/edu-2636-workqueue-slackbot-tut/docs/tutorials/typescript/work-queue-slack-app/code/temporal-application/src/__tests__/workqueue.test.ts)
```ts
...
In the root of the `bot` project, create a file `slack_bot.ts` and initialize the Slack bot similar to the following, making sure to import and initialize the Temporal Client module:
...
Otherwise, you will need to constantly visit the Temporal UI or use the Temporal CLI to terminate the Workflow.
...
By using the Slack channel name as the Workflow ID, it becomes easy to know which Workflow corresponds to which channel in the Temporal UI if you need to do any debugging.

![Workflow ID in Temporal UI](./images/workflow-id-temporal-ui.png)

### Display the Work Queue

To display the Work Queue, first we need to ensure that a Work Queue exists for this channel.
...
In the preceeding snippet, the `createNewWorkqueue` function attempts to start a new Work Queue Workflow.
...
The `queryWorkqueue` function then sends a Query the Work Queue Workflow for the channel and returns the data.
