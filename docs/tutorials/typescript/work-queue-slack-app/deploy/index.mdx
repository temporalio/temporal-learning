...
description: Learn how to deploy your TypeScript and Temporal based Slack App to production on a DigitalOcean Droplet using Ubuntu 20.04 or 22.04 and Temporal Cloud.
...
DigitalOcean provides flexible Cloud servers called Droplets that can be used to host your application.

In this tutorial we will walk through the steps to deploy your Temporal backed Slack App to a DigitalOcean Droplet while using Temporal Cloud as your Temporal Application's orchestrator.
...
- **A DigitalOcean account:** You need a DigitalOcean account to create a Droplet.
...
## Step 1 -- Create a Temporal Cloud Namespace certificate {#create-temporal-cloud-cert}

You will need a certificate to authenticate the communication between your Temporal Client, Temporal Workers, and Temporal Cloud.
...
## Step 2 -- Update code to use Temporal Cloud {#use-temporal-cloud}
...
### Update your Temporal Client {#update-temporal-client}
...
### Update Temporal Worker {#update-temporal-worker}
...
When using Temporal Cloud, your Temporal Worker should look like this:
...
### Update Environment variables {#update-env-vars}

You will need to update the `.env` files in both your `temporal-application` and `bot` projects to use the Temporal Cloud environment variables.

:::note Do this step locally first to test your Temporal Cloud connection

You will want to do this step locally first to ensure that your Slack bot and Temporal Application can connect to Temporal Cloud.

However, you will need to ensure that these same environment variables are set when you deploy your code to your DigitalOcean Droplet.

:::

#### Slack bot environment variables {#slack-bot-env-vars}

In addition to the existing environment variables in your `.env` file, you will need to add your Temporal Cloud Namespace, address, certificate PEM and KEY to the `.env` file.
...
Then update your `ENV` variable from `dev` to `prod`.

```plaintext
ENV="prod"
```

#### Temporal Application environment variables {#temporal-app-env-vars}

Your Temporal Application `.env` file also needs to be updated with the Temporal Cloud environment variables and have the `ENV` variable changed from `dev` to `prod`.
...
## Step 3 -- Create and set up Droplet {#create-droplet}
...
## Step 4 -- Configure your Slack App on the Droplet {#configure-app}
...
If your repo is public then the above steps will work.
However, if your repo is private, you will need to use an access token to clone the repo.

:::

Then ensure you have the respective `.env` files in each of the project roots with the correct environment variables.
...
:::note Automate your deployment with a Shell script
...
You should now have a TypeScript based Temporal Application running on a DigitalOcean Droplet using Temporal Cloud as your Temporal Application's orchestrator.
