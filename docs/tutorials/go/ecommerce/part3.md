...
keywords: [go, golang, temporal, sdk, tutorial, workflows, testing, ecommerce]
title: 'Building an eCommerce web app with Temporal and Go, Part 3: Testing'
last_update:
...
You can watch a video version of this tutorial as well:
...
Temporal's testing environment makes unit testing Workflows much less complex.  Sending Signals and Queries to the currently running Workflow with `RegisterDelayedCallback()` is a low-friction approach. 

Most importantly, Temporal's testing environment provides utilities for mocking Activities and testing logic that executes after a delay.  That makes it less complicated to unit test Workflows that depend on external services or Workflows that involve long timeouts.

In the [next tutorial](./part4.md), you'll build a RESTful API on top of a Temporal Workflow.
