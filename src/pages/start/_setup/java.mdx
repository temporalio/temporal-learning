...
<SetupStep code={
<>
<CodeSnippet language="bash">{`java -version`}</CodeSnippet>
</>
}>
...
<CodeSnippet language="bash">{`mvn archetype:generate -DgroupId=helloworkflow -DartifactId=temporal-hello-world\n  -DarchetypeArtifactId=maven-archetype-quickstart -DinteractiveMode=false`}</CodeSnippet>
...
<CodeSnippet language="bash">{`cd temporal-hello-world`}</CodeSnippet>
<CodeSnippet language="bash">{`gradle init --type java-application --project-name temporal-hello-world --package helloworkflow`}</CodeSnippet>
